...
cmp_ok(fmtjs(d2s(11.00071e-10)), 'eq', '1.100071e-09', 'js: 11.00071e-10 ok');
...
cmp_ok(fmtjs(d2s(1e-10)), 'eq', '1e-10', 'js: 1e-10 ok');
...
cmp_ok(fmtjs(d2s(1234567.89)), 'eq', '1234567.89', 'js: 1234567.89 ok');
...
cmp_ok(fmtjs(d2s(1.1234567890123456e25)), 'eq', '1.1234567890123457e+25', 'js: 1.1234567890123456e25 ok');
...
cmp_ok(fmtjs(d2s(9e125)), 'eq', '9e+125', 'js: 9e125 ok');
...
cmp_ok(fmtjs(d2s(9.123e125)), 'eq', '9.123e+125', 'js: 9.123e125 ok');
...
cmp_ok(fmtjs(d2s(91144e125)), 'eq', '9.1144e+129', 'js: 91144e125 ok');
...
cmp_ok(fmtjs(d2s(1e23)), 'eq', '1e+23', 'js: 9e125 ok');
...
if($mpfr) {
  for(1..2) {
    ...
    for my $exp(0..50) {
      ...
      my $num = $rand + 0;
      cmp_ok(fmtjs(d2s($num)), 'eq', Math::MPFR::nvtoa($num), "fmtjs() format agrees with nvtoa(): " . sprintf("%.17g", $num));
...
#   Failed test 'fmtjs() format agrees with nvtoa(): -0.0109871654910165'
#   at t/reformat.t line 142.
#          got: '-1.09871654910165e-02'
#     expected: '-0.0109871654910165'
